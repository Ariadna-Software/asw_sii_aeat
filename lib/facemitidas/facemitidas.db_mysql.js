...
var facemitidasDbAPI = {
                ...
                con.query(sql, function (err, registros) {
                    ...
                    con.query(sql, function (err, registros) {
                        ...
                        if (config.Axapta == "S") {
                            ...
                            // Son  emitidas 
                            sql += " WHERE [GDESVATNUM] = '" + nifEmisor + "'";
                            sql += " AND [VATNUM] = '" + nifReceptor + "'";
    ...
    createFacEmitidaFromReg: function (reg, con, done) {
        ...
        var facEmitida = {};
        var nueva = false;
        var nifReceptor = reg.REG_FE_CNT_NIF;
        if (!nifReceptor) nifReceptor = reg.REG_FE_CNT_IDOtro_ID;
                                ...
                                con.query(sql, function (err, data) {
                                    ...
                                    facemitidasDbAPI.createFacEmitidaFromReg(reg, con, function (err) {
                                        ...
                                        if (config.Axapta == "S") {
                                            ...
                                            sql += " WHERE [ARIADNAID] = " + reg.IDEnvioFacturasEmitidas;
                                            sqlsDb.execSQL(sql, function (err) {
                                                if (err) return callback(err);
                                                return callback();
                                            })
