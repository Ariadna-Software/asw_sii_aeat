...
var facemitidasDbAPI = {
    ...
    getPendientes: function (done) {
        comun.getConnectionCallback(function (err, con) {
            if (err) return done(err);
            var sql = "SELECT * FROM facemitidas WHERE Enviada = 0 OR (Enviada = 1 AND Resultado <> 'Correcto' AND Mensaje NOT LIKE '%COD: 3000%')";
            con.query(sql, function (err, facemitidas) {
                con.end(function (err2) {
                    if (err) return done(err);
                    done(null, facemitidas);
    ...
    envioFacEmitidasPendientes: function (done) {
        var prMessage = "";
