...
var facturaEmitidasSoap = require("./facturasRecibidas.soap");
...
router.get('/:IDEnvioFacturasEmitidas', function (req, res) {
    ...
    facturasRecibidasDb.getById(IDEnvioFacturasEmitidas, function (err, registros) {
        if (err) return res.status(500).send(err.message);
...
router.post('/', function (req, res) {
    ...
    facturasRecibidasDb.post(facturaEmitidas, function (err, registro) {
        if (err) return res.status(500).send(err.message);
        res.json(registro);
    });
});
...
router.put('/', function (req, res) {
    ...
    facturasRecibidasDb.put(facturaEmitidas, function (err, registro) {
        if (err) return res.status(500).send(err.message);
        res.json(registro);
    });
});
...
router.delete('/:IDEnvioFacturasEmitidas', function (req, res) {
    ...
    facturasRecibidasDb.delete(IDEnvioFacturasEmitidas, function (err, result) {
        if (err) return res.status(500).send(err.message);
        res.json(result);
    });
});
...
router.post('/envDb/:IDEnvioFacturasEmitidas', function (req, res) {
    ...
    facturasRecibidasDb.getById(IDEnvioFacturasEmitidas, function (err, registros) {
        ...
        var jsObject = facturasRecibidasDbToJs.emitidasDbToJs(registro);
