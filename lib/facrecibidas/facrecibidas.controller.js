...
router.get('/pendientes', function (req, res) {
    facrecibidasDb.getPendientes(function (err, grupos) {
        if (err) return res.status(500).send(err.message);
        res.json(grupos);
    });
});
...
router.get('/enviadas-incorrectas', function (req, res) {
    facrecibidasDb.getEnviadasIncorrectas(function (err, grupos) {
        if (err) return res.status(500).send(err.message);
        res.json(grupos);
    });
});
...
router.get('/enviadas-correctas', function (req, res) {
    facrecibidasDb.getEnviadasCorrectas(function (err, grupos) {
        if (err) return res.status(500).send(err.message);
        res.json(grupos);
    });
});
...
router.post('/enviar', function (req, res) {
    facrecibidasDb.envioFacRecibidasPendientes(function (err, mens) {
        if (err) return res.status(500).send(err.message);
        res.json(mens);
    });
});
