...
var facrecibidasDbAPI = {
    ...
    getPendientes: function (done) {
        comun.getConnectionCallback(function (err, con) {
            if (err) return done(err);
            var sql = "SELECT * FROM facrecibidas WHERE Enviada = 0 OR (Enviada = 1 AND Resultado <> 'Correcto' AND Mensaje NOT LIKE '%COD: 3000%')";
            con.query(sql, function (err, facrecibidas) {
                con.end(function (err2) {
                    if (err) return done(err);
                    done(null, facrecibidas);
    ...
    createFacRecibidaFromReg: function (reg, con, done) {
        ...
        var nifEmisor = reg.REG_IDF_NumSerieFacturaEmisor;
        if (!nifEmisor) nifEmisor = reg.reg.REG_IDF_NumSerieFacturaEmisor;
        ...
        con.query(sql, function (err, data) {
            facRecibida = {
                FacRecibidaId: 0,
                NifEmisor: nifEmisor,
                NombreEmisor: reg.REG_FE_CNT_NombreRazon,
    ...
    envioFacRecibidasPendientes: function (done) {
        var prMessage = "";
