...
var id = setInterval(function () {
    ...
    comun.getConnectionCallback(function (err, con) {
        ...
        var sql = "SELECT * FROM envio_facturas_emitidas WHERE EnvioInmediato = 1 AND Enviada = 0 LIMIT 10";
        con.query(sql, function (err, data) {
            ...
            async.eachSeries(data, function (reg, callback) {
                ...
                con.query(sql, function (err, data) {
                    ...
                    // enviamos el mensaje
                    console.log("-- [AEAT INICIO] --" + moment(new Date()).format("DD/MM/YYYY HH:mm:ss"));
                    facturaEmitidasSoap.sendXml(jsObject, function (err, result) {
                        if (err) {
                            console.log("-- [AEAT FIN CON ERROR] --" + moment(new Date()).format("DD/MM/YYYY HH:mm:ss"));
                            reg.Enviada = 1;
                            reg.resultado = "Error";
                            ...
                            console.log("-- [AEAT FIN NORMAL] --" + moment(new Date()).format("DD/MM/YYYY HH:mm:ss"));
