...
var facturasEmitidasSoapAPI = {
    sendXml: function (jsObject, done) {
        ...
        if (!cfg.wsdl || !cfg.wsdl.facturasEmitidas) {
            return done(new Error("Falta la configuración de la url WSDL para el envío de facturas"));
        }
        var urlWsdl = cfg.wsdl.facturasEmitidas;
        if (!cfg.xsd || !cfg.xsd.facturasEmitidas) {
            return done(new Error("Falta la configuración de la url XSD para el envío de facturas"));
        }
